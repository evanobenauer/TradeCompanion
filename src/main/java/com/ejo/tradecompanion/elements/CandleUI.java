...
public class CandleUI extends ElementUI {
    ...
    public CandleUI(Stock stock, double x, double focusY, double focusPrice, double width, Vector scale) {
        this(stock, StockUtil.getAdjustedCurrentTime(),x,focusY,focusPrice,width,scale);
    }
    ...
    private void updateData() {
        String[] rawData = getStock().getHistoricalData().get(getOpenTime().getDateTimeID());
        if (getOpenTime().equals(StockUtil.getAdjustedCurrentTime())) {
            data = new float[]{getStock().getOpen(), getStock().getPrice(), getStock().getMin(), getStock().getMax()};
        } else {
            data = new float[]{Float.parseFloat(rawData[0]), Float.parseFloat(rawData[1]), Float.parseFloat(rawData[2]), Float.parseFloat(rawData[3])};
        }
    }
